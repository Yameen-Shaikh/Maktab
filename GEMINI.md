...
### 2025-09-08
- Set a background image for the entire project by updating `base.html`.
- Corrected the location of static images by moving them from `students/templates/students/images` to `students/static/students/images`.
- Updated `base.html` to use the `static` template tag for the background image.
- Made the `login.html` template extend `base.html` to ensure a consistent look and feel.
- Configured `fees_manager/urls.py` to correctly serve static files during development.
- Added a `.gitignore` file with standard Python/Django ignores.